var Brain = function(kwargs) {
  ...
  _this.manifest = kwargs.manifest || [];
    ...
    _this.manifest = (kwargs.manifest || [])
    console.log("manifest", _this.manifest, kwargs, this.manifest)
    _this.manifest.forEach(function(val, idx, arr){
      _this.loadMesh(val.url, val.mesh_props);
      console.log("here")
    })
	...
	this.loadMesh = function(url, mesh_props) {
		...
		if (name_found && url == _this.meshes[mesh_props.roi_key].filename) {
			...
			oReq.onload = function(oEvent) {
				...
				mesh.material.opacity = mesh_props.opacity || 1;
